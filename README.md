...
- **Agent messages have typed renderers.** Markdown, HITL approval/reject/comment, suggested actions, editable Excel-like tables, Agent-to-user questions, mind maps, swimlanes and flowcharts each use an isolated component. Table messages support cell/header editing, row/column changes and pasted spreadsheet ranges; Q&A messages collect and submit user answers. Open **Preview Agent messages** on the generation page to exercise every renderer and interaction with representative data. Responses may provide one `message_payload` or a mixed `message_blocks` array.
