...
function agentMessageFromResponse(response, content, reasoning = "") {
  ...
  return {
    role: "agent",
    type: payload?.type || response?.message_type || "markdown",
    content: content ?? payload?.content ?? response?.message ?? "",
    data: payload?.data ?? response?.message_data,
    blocks: payload?.blocks ?? response?.message_blocks,
    reasoning,
...
function CaseGenerationPage({ cases, project, onAdd, onToast }) {
  ...
  function handleTurn(response, label, reasoning = "") {
    ...
    if (action === "ask") {
      ...
      setMessages((current) => [...current, agentMessageFromResponse(response, content, reasoning)]);
    ...
    setQuestions([]);
    setComposer("");
    setMessages((current) => [...current, agentMessageFromResponse(response, undefined, reasoning)]);
    if (action === "reply") {
      setStage("results");
      return;
    }
  ...
  async function askProjectSupervisor() {
    ...
    try {
      ...
      setMessages((current) => [...current, agentMessageFromResponse(response)]);
    } catch (reviewError) {
